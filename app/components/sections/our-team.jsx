...
export async function OurTeam({ data, sectionCount }) {
  const { fields } = data
  ...
  return (
    <Section className="relative" bg={bg}>
      <Container width={width} margin="section" className="relative z-1 p-14 border-2 border-secondary-950">
        <Motion>
          {/* title */}
...
const Teams = ({ items }) => (
  <dl className="grid text-left grid-cols-1 gap-8 lg:grid lg:grid-cols-4">
    {
      items?.map(item => (
        <MotionVariant variants={fadeInFromBottom} key={item.id}>
          <dt>
            <h1 className="mt-6 text-lg font-bold text-2xl tracking-tight text-gray-900">{_.get(item, 'fields.subtitle')}</h1>
          </dt>
          <dd className="mt-2 text-base leading-7 text-gray-600 uppercase tracking-wide">{_.get(item, 'name')}</dd>
        </MotionVariant>
      ))
    }
  </dl>
