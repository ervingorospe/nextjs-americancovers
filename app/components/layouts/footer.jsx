...
import { LocationCard, ButtonLink } from '@/components/layouts'
...
export function Footer({ navigation, locationInfo, location, socialMedia, general, logos }) {
  ...
  if (!_.get(navDetails, 'fields.hideFooter')) {
    return (
      <footer className="relative z-10 pt-12 pb-24 bg-secondary-100">
        <Container width="max-w-screen-lg" margin="default" className="text-center lg:text-left">
          ...
          if (componentName && _.get(item, 'fields.active')) {
            ...
            if (ComponentType) {
              return <ComponentType 
                ...
                styles={{
                  title: 'block font-heading text-2xl font-semibold text-gray-900',
                  contentText: 'mt-3 text-base text-gray-700',
                  link: 'hover:underline',
                  buttonStyle: 'inline-flex mt-3 button bg-primary-700 text-white hover:bg-primary-500 button-sm',
          ...
          if (_.get(item, 'parentId') === 0) {
            ...
            return (
              <li key={item.name}>
                <ButtonLink
                  navigation={navigation}
                  className="block text-base font-semibold tracking-wide text-gray-900 hover:text-secondary-900"
                  data={{
                    button: {
                        ...
                        subNav?.map((subItem, i) => {
                          return (
                            <li key={i}>
                              <ButtonLink
                                navigation={navigation}
                                className="text-sm text-gray-700 hover:text-secondary-900"
                                data={{
                                  button: {
...
const Logos = ({ logos }) => {
  return (
    <div className="mt-7">
          ...
          <div className="grid gap-y-8 md:flex">
            {
              logos?.map(image => (
                <Link href={_.get(image, 'fields.url')} key={image.id} className="mx-auto" target="_blank">
                  <Image className="h-20 md:h-32 w-auto" src={_.get(image, 'fields.image.imageUrl')} alt={defaultImageAlt} width={1000} height={1000}/>
                </Link>
              ))
            }
          </div>
      ...
      <p className="mt-5 flex items-center justify-center space-x-1 text-center text-sm text-secondary-900">
    ...
    <div className="mt-5 flex justify-center space-x-5">
      {
        socialMedia?.map((item, i) => {
          ...
          if (ComponentType)
            return (
              <ComponentType data={item} key={i} className="text-secondary-950 hover:text-secondary-800"/>
            )
        })
      }
    </div>
